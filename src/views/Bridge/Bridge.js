...
const Bridge = () => {
  ...
  console.log("sdfsdf",currentNetwork)
  ...
  const handleDeposit = async () => {
    ...
    if (fromChain !== SCALLOP_CHAINID) {
        ...
        deposit(amount, networkParams[toHex(toChain)].id, asset.resourceId,currentNetwork, fromChain, signer);
      

    } else {
      depositScallop(amount, networkParams[toHex(toChain)].id, asset.resourceId,currentNetwork, fromChain, signer);
    }
            ...
            >
              <Box component="div" sx={{ display: 'flex', "flex-direction":"row",justifyContent:"space-between" }}>
                <Box component="div" sx={{paddingLeft:"15px",paddingTop:"25px",display:"flex",flexDirection:"column"}}>
                  ...
                  <Typography>Bridge over scallopBridge for wEth wBNB wMatic</Typography>
                </Box>
